...
- 제목·소제목에 **핵심 키워드 포함**. 본문에만 키워드 있으면 ALF가 못 찾을 수 있음.
- 모호한·중복된 표현 지양 → ALF가 엉뚱한 문서를 참조할 위험
- 퍼블리시 상태만 참조됨 (수정 중 상태는 미참조)
- 이미지가 필요한 정보는 캡션으로 상세히 설명 (ALF는 이미지 자체가 아닌 캡션을 참조)
- 폴더 참조 설명과 일치하는 키워드 사용 → ALF가 폴더 진입 후 아티클 탐색
