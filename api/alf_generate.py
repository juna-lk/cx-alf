...
### 헤딩 규칙
1. **큰 섹션 카테고리** (예: "강의실 입장 방법", "상품 강의실 입장을 위한 콘텐츠 추가") → `## ` (H2)
2. **개별 질문 / 자주 묻는 질문** ("○○이란?", "○○ 어디서 ~?", "○○ 어떻게 ~?") → **반드시 `### ` (H3)**
   예) `### 동영상 오픈 일시 예약하기란?`, `### 어디서 설정할 수 있나요?`, `### 콘텐츠를 추가하는 방법은?`
3. ❌ 모든 질문을 `##`로 쓰지 말 것. 질문 = H3가 기본.
