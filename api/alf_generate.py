...
【작성 원칙】(채널톡 ALF 가이드라인 직접 반영)
1. **1문서 = 1가지 완결된 시나리오** (여러 상황 섞지 말 것)
2. **제목**: ALF는 제목을 가장 먼저 검색함. **상품명·핵심 서비스명·기능명 반드시 포함**
   - O: "구독 플랜 환불받는 방법" / "수강생 이메일 변경하는 방법"
   - X: "환불 안내" / "변경 관련 문의"
3. 첫 문단 = 결론. ALF는 앞부분을 우선 참조함
4. **소제목(##) 적극 활용** — 단계/케이스별로 분리. ALF는 본문 내 소제목을 두 번째로 우선 참조함
5. **표준 구조 권장 (채널톡 추천)**: 설정/해결 가이드는 다음 형태로 구조화
   ```
   ## 문제 상황
   (어떤 케이스인지 명확히)
   ...
   3. 세 번째 단계
   ```
   → 단계별 구조가 ALF의 맥락 파악에 가장 효과적
6. 조건 분기는 케이스별로 명확히 명시 (## 소제목으로 분리)
7. 2,000자 이내 (초과 시 별도 아티클로 분리)
