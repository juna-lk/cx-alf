...
## 마크다운 구조 강제 적용 (★★★ 가장 중요 — 절대 평문으로 출력 금지)
변환된 본문은 **반드시 마크다운 구조**를 갖춰야 합니다. 기존 채널톡 ALF 도큐먼트와 동일한 양식.

### 헤딩 규칙
1. **큰 섹션 카테고리** (5개 이상 질문 묶을 때만 사용. 예: "강의실 입장 방법", "결제·환불 안내") → `## ` (H2)
2. **개별 질문 / 자주 묻는 질문 / 케이스 안내** → **반드시 `### ` (H3)**
   - 명시적 질문형: `### 동영상 오픈 일시 예약하기란?`, `### 어디서 설정하나요?`
   - **암묵적 질문/케이스도 ### 분리** ★ — 평문 안내가 "왜 X가 표시되나요?"·"X가 없을 때 어떻게 되나요?"로 해석되면 그 자체를 ### 헤딩으로 추출
     · 원본: "아직 콘텐츠 오픈 전입니다 문구가 표시돼요. 해당 상품에 등록된 콘텐츠가 없는 경우 해당 문구가 표시될 수 있어요."
     · 변환: `### "아직 콘텐츠 오픈 전입니다" 문구는 언제 표시되나요?` + 답변 paragraph
3. ❌ 모든 질문을 `##`로 쓰지 말 것. 평문 paragraph 여러 개로 묶지 말 것. 각 케이스 = 별도 H3.

### 목록·강조 규칙
4. **단계/절차/항목 나열** → 반드시 `- ` 불릿 또는 `1. ` 번호 목록.
   - 메뉴 경로 라인 (예: "사이트 관리 > 상품 관리 > ...") → `- ` 불릿 항목으로.
   - 단계별 절차 (1단계 → 2단계 → ...) → `1. `, `2. ` 번호 목록으로.
5. **강조할 키워드 / 옵션명 / 버튼명 / 경로** → `**키워드**` (양옆에 공백 절대 금지)
   - O: `**상품 관리**`, `**[강의실 입장] 버튼**`
   - X: `** 상품 관리 **` (공백 들어가면 마크다운 굵게로 인식 안 됨)

### 출력 형식
6. 마크다운 raw 텍스트로 출력 (HTML 태그 직접 작성 X). `### 헤딩` 그대로, `<h3>` X.
7. **본문 전체가 평문 `<p>` paragraph로만 출력되면 실패**. 반드시 `##`·`###`·`-`·`1.`·`**` 중 하나 이상 포함.

## 변환 규칙 (★ 중요 — 매우 엄격하게 따를 것)
...
3. **🚨 매핑표에 없는 표현은 절대 변환 금지. 추측·유추·일관성 보강 변환 모두 금지.** 매핑표의 `old` 필드와 정확히 일치하는 표현만 변환.
4. **🚨 부분 매칭·복합 표현 부분 변환 절대 금지.** 매핑의 old가 본문의 일부 단어로 등장해도, 그 단어가 더 긴 의미 단위의 일부라면 변환 X.
   - 예시: "클래스 → 상품" 매핑 → 본문의 "클래스 게시판"·"클래스 단위"·"클래스 상세 페이지"·"클래스 강의실"·"클래스 신청"의 "클래스"는 **변환 금지** (복합 표현은 별도 매핑에 명시되어 있지 않으면 원본 그대로).
   - 예시: "강의 → 콘텐츠" 매핑 → "강의 자료"·"강의 오픈일"·"강의실"의 "강의"는 변환 금지.
   - **단어 경계 단독 등장(예: "클래스를 만들기")일 때만 매핑 적용. 다른 단어와 결합된 복합 표현은 보존.**
5. **applied_mappings에는 매핑표에 명시된 변환만 보고**. LLM이 자체 판단으로 한 변환은 보고 X (애초에 하면 안 됨).
6. **마크다운 형식 유지**. 헤딩(##), 불릿(-), 번호 목록(1.), 메뉴 경로 화살표(>) 등 본문 구조 보존.
7. **친근한 종결형(~어요/~합니다) 유지**. 본문 톤 변경 X.
8. 본문에 LK2.0 spec과 모순되는 정책이 있으면 변환하지 말고 warnings에 명시.

## 변환 금지 표현 (LK1.0 = LK2.0 동일, 절대 건드리지 말 것)
다음 표현은 LK1.0과 LK2.0에서 동일하게 사용됩니다. 매핑표에 없으므로 변환 금지:
- 수강생 (절대 "상품 신청자"·"구매자" 등으로 변환 X)
- 강의실, 동영상, 디지털 상품(콘텐츠 유형), 오프라인 모임, 라이브
- 결제 조회, 결제 취소, 환불, 부분 환불, 주문번호, 주문 상세 내역
- 회원, 운영자, 매니저, 강사
- 위에 명시되지 않았더라도 매핑표에 없는 모든 표현은 원본 그대로
