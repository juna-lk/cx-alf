...
변환된 본문은 **반드시 마크다운 구조**를 갖춰야 합니다. 채널톡이 markdown_to_html로 렌더링해서 admin에 헤딩·불릿·번호 목록 형태로 표시되어야 합니다.

1. **질문형 문장** ("○○이란?", "○○ 어떻게 하나요?", "○○ 어디서 ~?") → 반드시 **`## ` 헤딩**으로.
   예) `## 동영상 오픈 일시 예약하기란?`, `## 어디서 설정할 수 있나요?`
2. **단계/절차/항목 나열** → 반드시 **`- ` 불릿** 또는 **`1. ` 번호 목록**으로.
   - 메뉴 경로 라인 (예: "사이트 관리 > 상품 관리 > ...") → 그 자체를 `- ` 불릿 항목으로.
   - 단계별 절차 (1단계 → 2단계 → ...) → `1. `, `2. ` 번호 목록으로.
3. **강조할 키워드 / 옵션명 / 버튼명 / 경로** → **`**굵게**`** 처리.
   예) `**상품 관리**`, `**'동영상 오픈 일시 예약하기' 선택**`
4. **본문 전체가 평문 `<p>` paragraph로만 출력되면 실패**. 반드시 `##`·`-`·`1.`·`**` 중 하나 이상 포함.
5. 마크다운 raw 텍스트로 출력 (HTML 태그 직접 작성 X). `## 헤딩` 그대로, `<h2>` X.
