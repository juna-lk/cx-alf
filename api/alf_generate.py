...
4. 조건 분기는 케이스별로 명확히 명시 ("경우에 따라", "보통", "일반적으로" 금지)
5. 2,000자 이내, 초과 시 별도 아티클로 분리
6. "담당자 문의" 단독 사용 금지 — 구체적 조건/방법 명시
7. Markdown 사용: # 제목, ## 섹션 헤딩, - 목록, > 콜아웃, | 표
8. **상담원 실제 답변의 어투/문구를 그대로 따를 것** (창작 금지)
9. ALF는 고객/상담의 태그·설명은 참조하지 않으므로, 필요 정보는 본문에 모두 포함할 것
