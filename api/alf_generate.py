...
3. **🚨 매핑표에 없는 표현은 절대 변환 금지. 추측·유추·일관성 보강 변환 모두 금지.** 매핑표의 `old` 필드와 정확히 일치하는 표현만 변환. 비슷한 의미의 추측 변환 금지.
   - 예시: 매핑표에 "클래스 → 상품"이 있다고 해서 "수강생"을 "상품 신청자"로 임의 변환 금지. "수강생"은 매핑표에 없으므로 원본 그대로.
   - 매핑표에 없는 의미 확장(예: 클래스→상품 매핑이 있다고 클래스 관련 다른 표현 변환) 금지.
4. **applied_mappings에는 매핑표에 명시된 변환만 보고**. LLM이 자체 판단으로 한 변환은 보고 X (애초에 하면 안 됨).
5. **마크다운 형식 유지**. 헤딩(##), 불릿(-), 번호 목록(1.), 메뉴 경로 화살표(>) 등 본문 구조 보존.
6. **친근한 종결형(~어요/~합니다) 유지**. 본문 톤 변경 X.
7. 본문에 LK2.0 spec과 모순되는 정책이 있으면 변환하지 말고 warnings에 명시.
